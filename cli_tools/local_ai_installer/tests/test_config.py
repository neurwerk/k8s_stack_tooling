...
from local_ai_installer.downloader.errors import StorageUnavailableError
...
def test_settings_use_installer_storage_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("LOCAL_AI_INSTALLER_STORAGE_ROOT", "/external-media")
    monkeypatch.setenv("HF_HOME", "/external-media/huggingface")
    assert Settings().storage_root == Path("/external-media")
